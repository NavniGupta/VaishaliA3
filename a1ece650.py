...
def displayedges(graph_paths,graph_pathset,set_of_graph,node_list,node_dict,nodes_intersection, duplicate_paths):
            for a in range(0, len(node_dict)):
                ...
                segment_intersection = node_list.keys()[node_list.values().index(point_of_intersection)]
                path_p1 = node_list.keys()[node_list.values().index(p1)]
                path_p2 = node_list.keys()[node_list.values().index(p2)]
                path_p3 = node_list.keys()[node_list.values().index(p3)]
                path_p4 = node_list.keys()[node_list.values().index(p4)]
...
def min_vertex_cover():
    ...
    sys.stdout.write( "V ")
    sys.stdout.flush()
    sys.stdout.write(str(len(node_list))+'\n')
    sys.stdout.flush()
